        ...
        for ($ite = 1; $ite <= $noiter; $ite++) {
            ...
            for ($p = 0; $p < $nopat; $p++) {
                ...
                for ($i = 0; $i < $n; $i++) {
                    ...
                    $el[$i] = $yd[$p][$i] - $yr[$i];
                    // calcular el mas cercano a cero 
...
$obj = new Madaline("andBack", count($x[0]), count($yd[0]), count($x), 10, 0.00050, 1, $x, $yd, $algoritmo, $numNeuronaCapas, $funcion);



 $ent = $obj->entrenar();
//echo #echo print_r(json_encode($ent));
//echo print_r(rna::guardar($nombre, $tipo, json_encode($confi), json_encode($w), json_encode($u), json_encode($x), json_encode($yd)));
// $cont = 0;
